...
__updated__ = "2025-12-01 20:16:03"
###############################################################################
...
class LoggerSetup:
    ...
    def __init__(
        ...
        if not self.logger.handlers:
            ...
            if (modes & FormatMode.SYSLOG):
                pass
            if (modes & FormatMode.HTTP):
            if (modes & FormatMode.QUEUE):
            if (modes & FormatMode.MEMORY):
            if (modes & FormatMode.DATABASE):
            if (modes & FormatMode.CLOUD):
            if (modes & FormatMode.EXTERNAL):
