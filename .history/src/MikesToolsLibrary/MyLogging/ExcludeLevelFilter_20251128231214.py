...
class ExcludeLevelFilter(logging.Filter):
    ...
    # Use instance-level filter sets to avoid cross-instance contamination

    def __init__(self, initial_level: int | None = None, name=""):
        """
        Initializes the filter with an empty set of levels to exclude.
        :param name: Optional name for the filter.
        """
        super().__init__(name)
        # Initialize the per-instance filter set
        self.Filters = set()
        if isinstance(initial_level, int):
            self.Filters.add(initial_level)
