...
__updated__ = "2025-11-28 21:35:39"
###############################################################################
import logging
import sys
...
class LoggerSetup:
    ...
    def __init__(self, name: str = "MikesToolsLibrary", level: int = logging.DEBUG, logfile: str = "app.log"):
        ...
        if not self.logger.handlers:
            ...
            console_formatter = logging.Formatter(CustomFormatter.DEFAULT_FORMAT, datefmt="%Y-%m-%d %H:%M:%S" )
            console_handler.setFormatter(console_formatter )
            console_handler.setFormatter(CustomFormatter.DEFAULT_FORMAT)
            self.logger.addHandler(console_handler)

            # File handler
            file_handler = logging.FileHandler(logfile, encoding="utf-8")
            file_handler.setLevel(level)
            file_formatter = logging.Formatter(CustomFormatter.DEFAULT_FORMAT,
