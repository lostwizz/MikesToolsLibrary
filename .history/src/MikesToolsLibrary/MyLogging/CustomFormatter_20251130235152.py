...
__updated__ = "2025-11-30 23:51:52"
###############################################################################
...
###############################################################################
###############################################################################
class AppendArgsLogger(logging.Logger):
    """
    Custom Logger that appends all *args to the message string
    instead of using printf-style interpolation.
    """
    ...
    def _log(self, level, msg, args,
        ...
        if args:
            # Choose separator: newline or space
            sep = "\n" if getattr(self, "_newline", False) else " "
            appended = sep.join(str(a) for a in args)
            msg = f"{msg}{sep}{appended}"
            args = ()  # prevent logging from interpolating again

        super()._log(level, msg, args,
                    exc_info, extra, stack_info, stacklevel)

    def set_newline(self, enabled: bool = True):
        """Toggle newline separator for appended args."""
