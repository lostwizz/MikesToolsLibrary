...
__updated__ = "2025-11-30 20:32:41"
###############################################################################
...
class LoggerSetup:
    ...
    # -----------------------------------------------------------------
    @classmethod
    def addLevelExclude(self, level_to_exclude: int):
        # for handler in logger.handlers:
        #     handler.addFilter(ExcludeLevelFilter(level_to_exclude))
        ExcludeLevelFilter.addFilterLevel(level_to_exclude)
            
    # -----------------------------------------------------------------
    @classmethod
    def removeLevelExclude(self, level_to_remove:int):
        # for handler in logger.handlers:
        #     handler.removeFilter(ExcludeLevelFilter(level_to_remove))
        ExcludeLevelFilter.removeFilterLevel( level_to_remove)

            
    # -----------------------------------------------------------------
    @classmethod
    def showExcludeLevelFilter(self, logger):
        # for handler in self.logger.handlers:
        for handler in logger.handlers:
            for handlerFilter in handler.filters:
                
                # handlerFilter.
                print( handlerFilter.__class__.__name__, getattr(handlerFilter, "level_to_exclude", None))
        # return  ExcludeLevelFilter.Filters
    
    # -----------------------------------------------------------------
    @classmethod
    def x(self):
        print (ExcludeLevelFilter)
