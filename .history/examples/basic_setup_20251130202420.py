...
__updated__ = "2025-11-30 20:24:20"
###############################################################################
...
def checkCustomLevels():
    ...
    logger.notice("This is a NOTICE messagea")
...
def displayExcludeLevel():
    ...
    print( f"ExcludeLevelFilter {mylog.LoggerSetup.showExcludeLevelFilter(logger) }")

    l = logging._nameToLevel.get("NOTICE")
    print(f"level is {l=}")
    
    mylog.LoggerSetup.addFilterLevel( logger, logging._nameToLevel.get("NOTICE"))
    print( f"ExcludeLevelFilter {mylog.LoggerSetup.showExcludeLevelFilter(logger) }")

    logger.notice("This is a NOTICE message AFTER 2")

    print( ExcludeLevelFilter.Filters)
    ExcludeLevelFilter.removeFilterLevel( logger, logging._nameToLevel.get("NOTICE") )

    logger.notice("This is a NOTICE mes sage AFTER 3")
    # print( ExcludeLevelFilter.Filters)
    print( f"ExcludeLevelFilter {mylog.LoggerSetup.showExcludeLevelFilter(logger) }")
